...
Sendal *head, *tail, *newNode, *cur, *del;
...
int main(){
  string dataBaru[3] = {"POLO", "CASUAL", "41"};
  createCircularSingleLinkedList(dataBaru);

  printCircular();

  string data[3] = {"EIGER", "SENDAL GUNUNG", "40"};
  addFirst(data);

  printCircular();

  string data2[3] = {"ANDO", "CASUAL", "40"};
  addLast(data2);

  printCircular();

  string data3[3] = {"CARVIL", "CASUAL", "42"};
  addMiddle(data3, 5);
